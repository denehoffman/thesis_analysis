...
class BinnedRegularizedPlot(luigi.Task):
    ...
    def output(self):
        return [
            luigi.LocalTarget(
                Paths.plots
                / f'binned_regularized_fit_chisqdof_{self.chisqdof:.1f}_ksb_costheta_{self.ksb_costheta:.2f}{"mesons" if self.cut_baryons else "baryons"}_splot_{self.splot_method}_{self.nsig}s_{self.nbkg}b{"_phase_factor" if self.phase_factor else ""}_waves{self.waves}_lda_{self.lda}_gamma_{self.gamma}.png'
            ),
        ]
