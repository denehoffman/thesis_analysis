...
from thesis_analysis.utils import exp_pdf_single, get_sweights


class SPlot(luigi.Task):
    ...
    def output(self):
        ...
        return [
            ...
            luigi.LocalTarget(
                ...
                / Path(str(self.run_period))
                / Path(f'chisqdof_{self.chisqdof:.1f}')
                / f'splot_{self.n_sig}s_{self.n_bkg}b.pkl'
            ),
            luigi.LocalTarget(
                Paths.plots
                / f'splot_fit_{self.data_type}_{self.run_period}_chisqdof_{self.chisqdof:.1f}_splot_{self.n_sig}s_{self.n_bkg}b.png'
            ),
        ]

    def run(self):
        input_path = Path(self.input()[0][0].path)
        input_path_sigmc = Path(self.input()[1][0].path)
        input_path_bkgmc = Path(self.input()[2][0].path)
        output_path = Path(self.output()[0].path)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        output_fit_path = Path(self.output()[1].path)
        output_fit_path.parent.mkdir(parents=True, exist_ok=True)
        output_plot_path = Path(self.output()[2].path)
        ...
        fit_result, weights = get_sweights(
            data_df['RFL1'],
            data_df['RFL2'],
            data_df['Weight'],
            sigmc_df['RFL1'],
            sigmc_df['RFL2'],
            sigmc_df['M_Resonance'],
            sigmc_df['Weight'],
            bkgmc_df['RFL1'],
            bkgmc_df['RFL2'],
            bkgmc_df['M_Resonance'],
            bkgmc_df['Weight'],
            n_sig=self.n_sig,  # type: ignore
            n_bkg=self.n_bkg,  # type: ignore
        )

        fit_result.save(output_fit_path)
        ...
        sig_lines = [
            ...
            * fit_result.yields_sig[i]
            / z_total
            for i in range(fit_result.n_sig)
        ]
        ...
        bkg_lines = [
            ...
            * fit_result.yields_bkg[i]
            / z_total
            for i in range(fit_result.n_bkg)
        ]
