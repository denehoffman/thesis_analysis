...
class UnbinnedFitUncertainty(luigi.Task):
    ...
    def output(self):
        return [
            luigi.LocalTarget(
                Paths.fits
                / f'unbinned_fit_chisqdof_{self.chisqdof:.1f}_splot_{self.splot_method}_{self.nsig}s_{self.nbkg}b{"_guided" if self.guided else ""}{"_phase_factor" if self.phase_factor else ""}_waves{self.waves}_uncertainty_{self.uncertainty}_unbinned_bootstrap.pkl'
            ),
        ]
